...
class AudioEmotionModel(nn.Module):
    def __init__(self, d_model=257, num_classes=5, sample_rate=16000, patch_length_ms=32, hop_length_ms=10):
        ...
        self.alpha = nn.Parameter(torch.rand(1))  # 학습 가능한 가중치 파라미터
    ...
    def forward(self, src):
        ...
        #학습 가능한 가중치를 적용
        alpha = torch.sigmoid(self.alpha)  #alpha 값이 0과 1 사이가 되도록 보장
        src1_weighted = alpha * src1
        src2_weighted = (1 - alpha) * src2

        # 처리된 고주파(high frequency)와 저주파(low frequency) 부분을 합침
